...
Citizen.CreateThread( function()
    while true do
        ...
        if IsPedTryingToEnterALockedVehicle(GetPlayerPed(-1)) or IsPedJacking(GetPlayerPed(-1)) then
			...
			if PlayerData.job ~= nil and PlayerData.job.name == 'fib' and showcopsmisbehave == false then
			elseif PlayerData.job ~= nil and PlayerData.job.name == 'fib' and showcopsmisbehave then
				ESX.TriggerServerCallback('esx_outlawalert:ownvehicle',function(valid)
					if (valid) then
						...
						ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
							...
							if s2 == 0 then
								if IsPedInAnyfibVehicle(GetPlayerPed(-1)) then
									TriggerServerEvent('thiefInProgressS1fib', street1, vehName2, sex)
								else
									TriggerServerEvent('thiefInProgressS1', street1, vehName2, sex)
								end
							elseif s2 ~= 0 then
								if IsPedInAnyfibVehicle(GetPlayerPed(-1)) then
									TriggerServerEvent('thiefInProgressfib', street1, street2, vehName2, sex)
								else
