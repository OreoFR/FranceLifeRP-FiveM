...
local pizzeria = { x = -582.923, y = -407.704, z = 38.080 } --Configuration marker prise de service
local pizzeriafin = { x = -582.923, y = -405.279, z = 38.080 } --Configuration marker fin de service
local spawnfaggio = { x = -563.235, y = -389.551, z = 35.033 } --Configuration du point de spawn du faggio
    ...
    if isToHouse == true then
      ...
      if GetDistanceBetweenCoords(px,py,pz, GetEntityCoords(GetPlayerPed(-1),true)) < 3 then
        ...
        if IsControlJustPressed(1,38) then
          ...
          if (posibilidad > 120) and (posibilidad < 150) then

            pourboire = math.random(350, 450)
