...
function OpenCloakroomMenu()
  ...
  local elements = {
    ...
    if PlayerData.job.grade_name =='recruit' then 
    	table.insert(elements, {label =('Tenue Police National'), value = 'policenational_wear'})
    end
  
  	if PlayerData.job.grade_name =='officer' then 
    	table.insert(elements, {label =('Tenue Gendarme'), value = 'gendarme_wear'})
    ...
    if PlayerData.job.grade_name =='sergeantchief' then 
  	...
  	if PlayerData.job.grade_name =='boss' then  
	    table.insert(elements, {label =('Tenue Gendarme'), value = 'gendarme_wear'})
    	table.insert(elements, {label =('Tenue Colonel'), value = 'colonel_wear'})
		table.insert(elements, {label =('Tenue Grand Froid Gendarme'), value = 'grandfroidgendarmerie_wear'})
		table.insert(elements, {label =('Tenue Grand Froid Gendarme 2'), value = 'grandfroidgendarmerie2_wear'})
		table.insert(elements, {label =('Tenue Gendarmerie Maitre Chien'), value = 'maitrechien_wear'})
		table.insert(elements, {label =('Tenue Pilote Gendarmerie'), value = 'pilotedegend_wear'})
        table.insert(elements, {label =('Tenue RAID'), value = 'raid_wear'})
        table.insert(elements, {label =('TENUE OREO PAS TOUCHER'), value = 'oreo_wear'})
    end 
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      if data.current.value == 'gendarme_wear' then
        ...
        if skin.sex == 0 then
          ...
          SetModelAsNoLongerNeeded(model)
            ...
            if data.current.value == 'pilotedegend_wear' then
      ...
      if data.current.value == 'policenational_wear' then
        ...
        if skin.sex == 0 then
          ...
          SetModelAsNoLongerNeeded(model)
            ...
            if data.current.value == 'oreo_wear' then

        ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
        TriggerServerEvent("player:serviceOn", "police")

        if skin.sex == 0 then
          local model = GetHashKey("a_m_y_runner_01")
    ...
    if data.current.value == 'grandfroidgendarmerie_wear' then
...
function OpenVehicleSpawnerMenu(station, partNum)
  ...
  if Config.EnableSocietyOwnedVehicles then
    ...
    if PlayerData.job.grade_name == 'officer' then
      table.insert(elements, { label = 'Dacia Gendarmerie', value = 'sheriff2'})
  ...
  if PlayerData.job.grade_name == 'sergeantchief' then
    ...
    if PlayerData.job.grade_name == 'lieutenant' then
	  ...
	  table.insert(elements, { label = 'Megane RS Gendarmerie', value = 'police2'})
    table.insert(elements, { label = 'Kangoo Gendarmerie', value = 'pranger'})
	  table.insert(elements, { label = 'PSIG', value = 'fbi'})
	  table.insert(elements, { label = 'Dacia Gendarmerie', value = 'sheriff2'})
	  table.insert(elements, { label = 'Moto Gendarmerie', value = 'policeb'})
	  table.insert(elements, { label = '508 Banalisé', value = 'police4'})
    end
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlPressed(0, Keys['F10']) and not isDead and PlayerData.job ~= nil and PlayerData.job.name == 'police' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'police_actions') and (GetGameTimer() - GUI.Time) > 150 then
			OpenPoliceActionsMenu()
			GUI.Time = GetGameTimer()
		end
