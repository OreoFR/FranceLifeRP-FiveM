...
function OpenMecanoActionsMenu()
  ...
  local elements = {
    ...
    {label = _U('work_wear3'), value = 'cloakroom3'},
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data, menu)
            ...
            if amount == nil then
              ...
              menu.close()
              TriggerServerEvent('esx_society:washMoney', 'society_mecano', amount)
            end
          end,
          function(data, menu)
            menu.close()
            ...
            local elements = {
			  ...
			  {label = _U('dir_vehicle'), value = 'utillitruck3'}
            }
              ...
              function(data, menu)
                for i=1, #elements, 1 do
                  if Config.MaxInService == -1 then
                    ESX.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, 216.0, function(vehicle)
                      local playerPed = GetPlayerPed(-1)
                      TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
                    end)
                    break
      ...
      if data.current.value == 'cloakroom3' then
            ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
              TriggerServerEvent("player:serviceOn", "mecano")
              local playerPed = GetPlayerPed(-1)
              if skin.sex == 0 then
                SetPedComponentVariation(playerPed, 8, 15, 0, 0) --T-shirt
                SetPedComponentVariation(playerPed, 11, 66, 0, 0) --Torse
                SetPedComponentVariation(playerPed, 10, 0, 0, 0) --Decals
                SetPedComponentVariation(playerPed, 3, 46, 0, 0) --Bras
                SetPedComponentVariation(playerPed, 4, 39, 0, 0) --Pantalon
                SetPedComponentVariation(playerPed, 6, 24, 0, 0) --Chaussures
                SetPedComponentVariation(playerPed, 7, 0, 0, 0) --Collier
                SetPedPropIndex(playerPed, 0, 110, 4, 0) -- Helmet
      ...
      if data.current.value == 'boss_actions' then
        TriggerEvent('esx_society:BossMenuOpen1', 'mecano', function(data, menu)
          menu.close()
        end)
      end
            ...
            if CurrentAction == 'delete_vehicle' then
              ...
              if Config.EnableSocietyOwnedVehicles then
                ...
                if
				  ...
				  GetEntityModel(vehicle) == GetHashKey('utillitruck3') or
