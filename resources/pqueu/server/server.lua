...
Config.Language = {
    ...
    idrr = "Erreur: Impossible de récupérer vos identifiants, essayez de redémarrer.",
    err = "Il y avait une erreur",
    pos = "[Patience...] 🛫Vous êtes% d /% d en attente d'un avion🛬",
    connectingerr = "Erreur lors de l'ajout à la liste de Connection",
    timedout = "Timed out?",
    wlonly = "Vous devez être en liste blanche pour rejoindre ce serveur",
    banned = "Vous êtes banni | les raisons",
    steam = "Erreur: Steam doit être en cours d'exécution"
}
-----------------------------------------------------------------------------------------------------------------------
...
function Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    for k,v in ipairs(self.QueueList) do
        if tmp.priority then
            ...
            if _pos then
                self:DebugPrint(string_format("%s[%s] a été priorisé et placé %d/%d dans l'avion", tmp.name, ids[1], _pos, queueCount))
                break
            end
        end
    end
    ...
    if not _pos then
        _pos = self:GetSize() + 1
        self:DebugPrint(string_format("%s[%s] était placé %d/%d dans l'avion", tmp.name, ids[1], _pos, queueCount))
    end
...
Citizen.CreateThread(function()
    local function playerConnect(name, setKickReason, deferrals)
        ...
        if Queue:IsInQueue(ids) then
            ...
            Queue:DebugPrint(string_format("%s[%s] a rejoint l'avion après l'annulation", name, ids[1]))
        else
        ...
        if Queue:NotFull(true) then
            ...
            Queue:DebugPrint(name .. "" .. ids[1] .. " le chargement de la ville")
    ...
    local function checkTimeOuts()
        ...
        while i <= Queue:GetSize() do
            ...
            if not data.ids or not data.name or not data.firstconnect or data.priority == nil or not data.source then
                data.deferrals.done(Config.Language.err .. "")
            ...
            elseif (data.timeout >= 120) and data.source ~= "debug" and os_time() - data.firstconnect > 5 then
                ...
                Queue:DebugPrint(data.name .. "[" .. data.ids[1] .. "] a été retiré de la file d'attente, car ils ont expiré")
            else
                i = i + 1
            end
        end
        ...
        while i <= Queue:ConnectingSize() do
            ...
            if ((data.timeout >= 300 and lastMsg >= 35000) or data.timeout >= 340) and data.source ~= "debug" and os_time() - data.firstconnect > 5 then
                Queue:RemoveFromQueue(data.source, true)
                Queue:RemoveFromConnecting(data.source, true)
                Queue:DebugPrint(data.name .. "[" .. data.ids[1] .. "] a été retiré de la file d'attente de connexion, car ils ont expiré")
            else
                i = i + 1
            end
        end
